...
export function ReportsWorkspace({ newReportSignal = 0 }: { newReportSignal?: number }) {
  ...
  useEffect(() => {
    if (!editing) {
      if (reports.length > 0) setEditing(reports[0]);
      else if (clientId) newReport();
    }
  }, [reports, clientId]);
