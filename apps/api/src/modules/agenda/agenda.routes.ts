...
export const agendaRoutes: FastifyPluginAsync = async (app) => {
    ...
    if (scope.mode !== "global") {
      if (scope.clientIds.length > 0) {
        scopeSql = ` AND (
          ...
          OR e.client_account_id IN (${scope.clientIds
            .map(() => "?")
            .join(", ")})
        )`;
    ...
    const [rows] = await app.db.query(
      `
        SELECT
          ...
          e.agenda_label_id AS labelId,
          a.name AS clientName,
          l.name AS labelName
        FROM agenda_events e
        LEFT JOIN client_accounts a
          ON a.id = e.client_account_id
        LEFT JOIN agenda_labels l
          ON l.id = e.agenda_label_id
        WHERE
          e.starts_at < ?
          AND (
  ...
  app.delete("/agenda/labels/:labelId", async (request) => {
    ...
    await app.db.query(
      `
        UPDATE agenda_events
        SET agenda_label_id = NULL
        WHERE agenda_label_id = ?
      `,
      [labelId],
    );
